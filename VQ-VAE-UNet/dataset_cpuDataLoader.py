...
class readAudio():
    ...
    def __getitem__(self, num_sets, chunk_size, rand_amp=False, min_amp = 0.7, max_amp = 1.0):
...
######################################################################################################
        ...
        for index in range(num_sets):
            ...
            if rand_amp == True:
                print(f'Randomly changing the level of source amp by [{min_amp}, {max_amp}]')
                # out_l_mix_uncut = data_mix[:, 0].astype(float)
                out_l_bass_uncut = data_bass[:, 0].astype(float) * np.random.uniform(min_amp, max_amp)
                out_l_drums_uncut = data_drums[:, 0].astype(float) * np.random.uniform(min_amp, max_amp)
                out_l_vocals_uncut = data_vocals[:, 0].astype(float) * np.random.uniform(min_amp, max_amp)
                out_l_other_uncut = data_other[:, 0].astype(float) * np.random.uniform(min_amp, max_amp)
                # out_r_mix_uncut = data_mix[:, 1].astype(float)
                out_r_bass_uncut = data_bass[:, 1].astype(float) * np.random.uniform(min_amp, max_amp)
                out_r_drums_uncut = data_drums[:, 1].astype(float) * np.random.uniform(min_amp, max_amp)
                out_r_vocals_uncut = data_vocals[:, 1].astype(float) * np.random.uniform(min_amp, max_amp)
                out_r_other_uncut = data_other[:, 1].astype(float) * np.random.uniform(min_amp, max_amp)
                
                out_l_mix_uncut = out_l_bass_uncut + out_l_drums_uncut + out_l_vocals_uncut + out_l_other_uncut
                out_r_mix_uncut = out_r_bass_uncut + out_r_drums_uncut + out_r_vocals_uncut + out_r_other_uncut
            
            else:
                print('No Randomly changing the level of source amp')
                out_l_mix_uncut = data_mix[:, 0].astype(float)
            ...
            # for start in range(1000000,1000001): # debug
                ...
                #     if len(out_r_mix) == chunk_size and len(out_r_drums) == chunk_size:
                        ...
                        # print('out_l_mix.shape', out_l_mix.shape) # out_l_mix.shape torch.Size([132300])
